...
async function main() {
  ...
  console.log('Connected. Total chars: ' + totalChars + ', Scenes: ' + scenes.length);

  // Verify Peter
  const voice = await evaluate(socket, `document.body.innerText.includes('Peter') ? 'PETER' : 'NOT_PETER'`);
  console.log('Voice: ' + voice);
  if (voice !== 'PETER') { console.log('ABORT'); socket.close(); process.exit(1); }

  for (let i = 0; i < scenes.length; i++) {
    const { num, text } = scenes[i];
    console.log(`\nScene ${String(num).padStart(2,'0')} (${text.length} chars)`);

    // Clear existing text by selecting all and deleting
    await evaluate(socket, `
      (function() {
        const btn = document.querySelector('[aria-label="Clear text"]');
    ...
    // Click on the text input area (coordinates from earlier: ~550, 120)
    await cdp(socket, 'Input.dispatchMouseEvent', {type:'mousePressed', x:400, y:120, button:'left', clickCount:1});
    await cdp(socket, 'Input.dispatchMouseEvent', {type:'mouseReleased', x:400, y:120, button:'left', clickCount:1});
    await sleep(500);

    // Insert text via CDP
    await cdp(socket, 'Input.insertText', {text: text});
    await sleep(2000);

    // Click Generate via aria-label
    let generated = false;
    for (let attempt = 0; attempt < 5; attempt++) {
      const result = await evaluate(socket, `
        (function() {
          const btn = document.querySelector('[aria-label*="Generate speech"]');
          if (btn && !btn.disabled) { btn.click(); return 'ok'; }
          if (btn && btn.disabled) return 'disabled';
          return 'missing';
        })()
      `);
      if (result === 'ok') { generated = true; break; }
      console.log('  Retry generate: ' + result);
      await sleep(2000);
    }
    ...
    for (let w = 0; w < 30; w++) {
      ...
      const status = await evaluate(socket, `
        (function() {
          ...
          return credits + '|' + (btn ? btn.disabled : 'x');
        })()
      `);
  ...
  const final = await evaluate(socket, `
    (function() {
      const body = document.body.innerText;
      const m = body.match(/(\\d[\\d,]+)\\s*credits/);
      return m ? m[1] + ' credits' : 'unknown';
    })()
  `);
  console.log('\n=== COMPLETE === ' + final);
  socket.close();
